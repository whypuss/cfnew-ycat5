    ...
    // Route randomization mappings (generated by build.js)
    const RANDOMIZED_ROUTES = { '/sub': '/trvx', '/api/config': '/ovr/jckn', '/api/preferred-ips': '/htr/ogudp', '/?ed=2048': '/v2?ed=2048' };
    ...
    const FAKE_RESPONSE_HEADERS = { 'x-build': 'x-edge-vyw', 'x-edge': 'x-edge-qka', 'x-runtime': 'x-edge-bxs', 'server-timing': 'x-build-iul' };
    const ROUTE_ALIASES = ['/qqt', '/agg', '/mvge'];
    const QUERY_PARAM_ALIASES = { 'target': 'bvr', 'token': 'kpti', 'wk': 'uyt' };
...
`;
    ...
    const SCHEDULED_HEALTH_INTERVAL = 900; // 15 minutes in seconds
    ...
    // P1-2: Three-layer health check: TCP connect → TLS handshake → WebSocket upgrade
    // Returns {tcp: bool, tls: bool, ws: bool, latency: ms}
    async function threeLayerHealthCheck(ip, port) {
        const result = { tcp: false, tls: false, ws: false, latency: 0 };
        const startTime = Date.now();

        try {
            // Layer 1: TCP connect with 3s timeout
            const tcpController = new AbortController();
            const tcpTimeout = setTimeout(() => tcpController.abort(), 3000);

            try {
                const tcpSocket = connect({ hostname: ip, port: port });
            ...
            let tlsSocket;
            try {
                tlsSocket = tcpSocket.startTls({ hostname: ip });
                await tlsSocket.opened;
                result.tls = true;
            } catch (e) {
                // TLS failed, close TCP socket
                try { tcpSocket.close(); } catch (_) {}
                result.latency = Date.now() - startTime;
                return result;
            }

            // Layer 3: WebSocket upgrade attempt
            try {
                // Send WebSocket upgrade request
                const wsRequest = `GET / HTTP/1.1\r\nHost: ${ip}:${port}\r\nUpgrade: websocket\r\nConnection: Upgrade\r\nSec-WebSocket-Key: dGhlIHNhbXBsZSBub25jZQ==\r\nSec-WebSocket-Version: 13\r\n\r\n`;
                const encoder = new TextEncoder();
                await tlsSocket.write(encoder.encode(wsRequest));

                // Read response (with timeout)
                const wsController = new AbortController();
                const wsTimeout = setTimeout(() => wsController.abort(), 2000);
                ...
                try {
                    ...
                    while (true) {
                        ...
                        if (done) break;
                        wsResponse += new TextDecoder().decode(value);
                        if (wsResponse.includes('\r\n\r\n')) break;
                    }
                    wsController.abort();
                } catch (e) {}
                ...
                if (wsResponse.includes('101') && wsResponse.toLowerCase().includes('switching protocols')) {
                    result.ws = true;
                }
            } catch (e) {
                // WebSocket upgrade failed, but TLS succeeded
            }
    ...
    // P1-2: Run health check on all known nodes and update KV
    async function runScheduledHealthCheck(env) {
        ...
        try {
            // List all node keys from KV
            const list = await kvStore.list({ prefix: NODE_RECORD_PREFIX });
            const nodeKeys = list.keys || [];

            for (const key of nodeKeys) {
                try {
                    // Parse ip:port from key
                    const keyName = key.name || '';
                    const parts = keyName.replace(NODE_RECORD_PREFIX, '').split(':');
                    ...
                    // Skip quarantined nodes unless they're being rechecked
                    const record = await getNodeRecord(ip, port);
                    if (record?.quarantined) {
                        // Recheck quarantined nodes but don't unquarantine automatically
                        // They need manual review or sustained good performance
                        continue;
                    }

                    // Run three-layer health check
                    const health = await threeLayerHealthCheck(ip, port);
                    checkedCount++;

                    if (health.tcp) {
                        // Update reputation based on health check results
                        const success = health.tls; // Consider TLS success as overall success
                        await updateNodeReputation(ip, port, 'health-check', success, health.latency);
            ...
            for (const node of getBackupIPs()) {
                ...
                if (!isValidIP(node.domain)) {
                    // This is a domain, try to check it differently
                    // For now, skip domains as they require DNS resolution
                    continue;
                }

                const ip = node.domain;
                ...
                if (health.tcp) {
                    const success = health.tls;
                    await updateNodeReputation(ip, port, node.source || 'CMLiussss', success, health.latency);
                    if (success) successCount++;
                }
            }
            ...
            console.log(`[HealthCheck] Completed in ${elapsed}ms. Checked: ${checkedCount}, Success: ${successCount}`);

        } catch (e) {
            console.error('[HealthCheck] Error running scheduled health check:', e);
        }
    }
        ...
        async fetch(request, env, ctx) {
            try {
                ...
                if (request.method === 'GET') {
                    ...
                    // P2-1: XML sitemap for SEO normalization
                    if (pathname === '/sitemap.xml') {
                        const sitemap = `<?xml version="1.0" encoding="UTF-8"?>
<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">
  <url><loc>https://cfnew.agooxo.workers.dev/</loc><changefreq>daily</changefreq><priority>1.0</priority></url>
  <url><loc>https://cfnew.agooxo.workers.dev/welcome</loc><changefreq>weekly</changefreq><priority>0.8</priority></url>
</urlset>`;
...
<browserconfig>
    <msapplication>
        <tile>
            ...
            <TileColor>#6ab5d0</TileColor>
                ...
                if (url.pathname.includes(RANDOMIZED_ROUTES['/api/config'])) {
                ...
                if (url.pathname.includes(RANDOMIZED_ROUTES['/api/preferred-ips'])) {
                    const pathParts = url.pathname.split('/').filter(p => p);
                    const apiIndex = pathParts.indexOf('api');
                    if (apiIndex > 0) {
                        const pathSegments = pathParts.slice(0, apiIndex);
    ...
    <body>
        ...
        async scheduled(controller, env, ctx) {
            // P1-2: Run periodic health checks every SCHEDULED_HEALTH_INTERVAL (15 min)
            // Cloudflare Cron Triggers invoke this based on schedule defined in wrangler.toml
            try {
                await initKVStore(env);
                await runScheduledHealthCheck(env);
            } catch (err) {
    ...
    <body>
        ...
        <script>
            ...
            function generateClientLink(clientType, clientName) {
                ...
                // NOTE: browser-side sub path (injected by build.js from RANDOMIZED_ROUTES['/sub'])
