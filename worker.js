    ...
    const RANDOMIZED_ROUTES = { '/sub': '/syv', '/connect': '/data' };
...
<browserconfig>
        ...
        <script>
            ...
            if (cp && cp.trim()) {
                ...
                const normalizedPath = url.pathname.endsWith('/') && url.pathname.length > 1 ? url.pathname.slice(0, -1) : url.pathname;
                    ...
                    // P2-0: Direct alias shortcut — /zakx without UUID prefix → subscription page
                    if (hasSubRoute(url.pathname)) {
                        return await handleSubscriptionPage(request, at);
                    }
    ...
    function parseLinkToClashNode(link) {
        try {
            ...
            if (link.startsWith('vless://')) {
                ...
                const path = params.get('path') || RANDOMIZED_ROUTES['/connect'];
        ...
        try {
            if (link.startsWith('vless://')) {
                ...
                return {
                    ...
                    path: p.get('path') || RANDOMIZED_ROUTES['/connect'],
    ...
    function generateLinksFromSource(list, user, workerDomain, echConfig = null, skipNumbering = false, source = 'unknown') {
        ...
        const links = [];
        const wsPath = RANDOMIZED_ROUTES['/connect'];
        const proto = atob('dmxlc3M=');
